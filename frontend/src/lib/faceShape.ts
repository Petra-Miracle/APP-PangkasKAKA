...
export function classifyFaceShape(facePoints: Point[]): FaceShapeResult | null {
  ...
  if (lengthToWidthRatio > 1.55) {
    shape = "oblong";
    marginScore = Math.min(1, (lengthToWidthRatio - 1.55) / 0.35);
  } else if (jawVsCheek > 0.14 && Math.abs(foreheadVsCheek) < 0.12) {
    shape = "heart";
    marginScore = Math.min(1, (jawVsCheek - 0.14) / 0.15);
  } else if (Math.abs(jawVsCheek) < 0.09 && Math.abs(foreheadVsCheek) < 0.09) {
    if (lengthToWidthRatio < 1.15) {
      shape = "round";
      marginScore = Math.min(1, (1.15 - lengthToWidthRatio) / 0.2);
    } else {
      shape = "square";
      marginScore = Math.min(1, (lengthToWidthRatio - 1.15) / 0.25);
    }
  } else {
    shape = "oval"; // balanced/default bucket when nothing else stands out
    marginScore = 0.6;
  }
