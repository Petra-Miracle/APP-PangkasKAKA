...
export default function ProductPaymentStatusScreen() {
  ...
  const doFallbackCheck = async () => {
    setChecking(true);
    try {
      await api.post(`/payments/fallback-check-product/${orderId}`);
      await fetchStatus();
    } catch {}
    setChecking(false);
  };
...
function PendingCard({ mm, ss }: { mm: number; ss: number }) {
  return (
    ...
    >
      ...
      <Text style={styles.stateSub}>Selesaikan pembayaran sebelum waktu habis. Pesanan akan dibatalkan otomatis jika tidak dibayar dalam 15 menit.</Text>
    </LinearGradient>
  );
}
